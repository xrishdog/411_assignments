...
class BattleModel:
    ...
    def battle(self) -> str:
        ...
        logger.info("Battle started between %s and %s",
                    combatant_1.meal, combatant_2.meal)
    ...
    def get_battle_score(self, combatant: Meal) -> float:
        ...
        # Calculate score
        score = (combatant.price * len(combatant.cuisine)) - \
            difficulty_modifier[combatant.difficulty]
    ...
    def get_combatants(self) -> List[Meal]:
        """
        Returns the current list of combatants (meals)
        """
    ...
    def prep_combatant(self, combatant_data: Meal):
        ...
        if len(self.combatants) >= 2:
            logger.error(
                "Attempted to add combatant '%s' but combatants list is full", combatant_data.meal)
            raise ValueError(
                "Combatant list is full, cannot add more combatants.")

        # Log the addition of the combatant
        logger.info("Adding combatant '%s' to combatants list",
                    combatant_data.meal)

        self.combatants.append(combatant_data)

        # Log the current state of combatants
        logger.info("Current combatants list: %s", [
